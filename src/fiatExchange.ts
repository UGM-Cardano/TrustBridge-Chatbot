...
let freecurrencyapiClient: any = null;
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function getFreecurrencyClient(): Promise<any> {
  if (!freecurrencyapiClient) {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const mod: any = await import('@everapi/freecurrencyapi-js');
...
const fiatCache = new Map<string, { rates: Record<string, number>; timestamp: number }>();
...
async function fetchLatest(base = 'USD'): Promise<Record<string, number>> {
  const cacheKey = base;
  ...
  try {
    ...
    const rates: Record<string, number> = response.data || {};
...
export async function convertFiat(amount: number, from: string, to: string): Promise<number> {
  const rate = await getFiatRate(from, to);
  return amount * rate;
}

export function clearFiatCache(): void {
  fiatCache.clear();
}

export function getFiatCacheStats(): { size: number; keys: string[] } {
