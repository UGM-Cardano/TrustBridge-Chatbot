...
try {
    ...
    if (ws.verifySignature)
        verifySignature = ws.verifySignature;
    if (ws.extractSignature)
        extractSignature = ws.extractSignature;
}
catch (err) {
    // leave stubs in place
    // logger isn't imported at top of file for this try/catch block safety
}
export function createWebhookRouter(whatsappClient) {
 ...
 */
async function handleTransactionUpdate(client, payload) {
    try {
